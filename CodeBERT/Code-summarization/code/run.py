    ...
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            if 'idx' not in js:
                js['idx'] = idx
            try:
                code_tokens = [temp.value for temp in list(javalang.tokenizer.tokenize(js['code']))]
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        # source
        clean_source = remove_invalid_characters(example.source)
        source_tokens = tokenizer.tokenize(clean_source)[:args.max_source_length - 2]
...
def test(args, model, tokenizer, device, epoch=0):
    ...
    model.train()
    predictions = []
    with open(os.path.join(args.output_dir, "test_{}.output".format(str(epoch))), 'w',encoding='utf-16') as f, open(
    ...
    dev_bleu = []
    dev_bleu_list = bleu.bleuListFromMaps(goldMap, predictionMap)
    with open(os.path.join(args.output_dir, "predictions.txt"), 'w') as f:
        for example, pred in zip(eval_data.examples, dev_bleu_list):
            f.write(f'{example.example_id}\t{pred[0]}\n')
            dev_bleu.append(pred[0])
    logger.info("  %s = %s " % ("single bleu-4", str(np.mean(dev_bleu))))
    logger.info("  " + "*" * 20)
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                output_model_file = os.path.join(last_output_dir, "codebert_sum_wir_model.bin")
                ...
                if eval_loss < best_loss:
                    ...
                    model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                    output_model_file = os.path.join(output_dir, "codebert_sum_wir_model.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
    ...
    print(f"训练时间为：{execution_time/60.0} 分钟")
