...
def main():
    ...
    query_times = 0
    for index, example in tqdm(enumerate(eval_dataset), total=len(eval_dataset)):
        print("Index: ", index)
        ...
        if len(adv_code) > 30000:
            ...
            recoder.write(index, first_code, adv_code_part, true_label, orig_label, temp_label, round((time.time() - start_time) / 60, 2))
            remaining_adv_code = adv_code[30000:]
            recoder.write("", "", remaining_adv_code, "", "", "", "")
        else:
            recoder.write(index, first_code, adv_code, true_label, orig_label, temp_label, round((time.time() - start_time) / 60, 2))
