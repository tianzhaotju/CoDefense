...
def main():
    ...
    for index, example in tqdm(enumerate(eval_examples), total=len(eval_examples)):
        ...
        if len(identifiers) == 0:
            recoder.write('-1', code, code, ref_summary, pre_summary, pre_summary,
        ...
        if is_success == 1:
            success_attack += 1
        recoder.write(is_success, code, adv_code, ref_summary, pre_summary, adv_summary, round((time.time() - start_time) / 60, 2))
