...
def test(args, model, tokenizer, device, epoch=0):
    ...
    model.train()
    predictions=[]
    with open(os.path.join(args.output_dir,"test_{}.output".format(str(epoch))),'w',encoding='utf-16') as f, open(os.path.join(args.output_dir,"test_{}.gold".format(str(epoch))),'w') as f1:
        for ref,gold in zip(p,eval_examples):
            predictions.append(str(gold.idx)+'\t'+ref)
            f.write(str(gold.idx)+'\t'+ref+'\n')
            f1.write(str(gold.idx)+'\t'+gold.target+'\n')     
